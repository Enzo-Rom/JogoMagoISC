.data
# Sprites dos cora��es (8x8, 1 byte/pixel RGB332, 0x00 = transparente)
coracao_cheio:
    .word 8, 8, 0
...
mana_atual:       .word 65        # valor de exemplo s� pra visualiza��o
hud_mana_x:       .word 4
hud_mana_y:       .word 14
...
# Fun��o da vida do player: desenha os cora��es do HUD
draw_hearts:
    addi sp, sp, -16
    sw ra, 12(sp)
    sw s1, 8(sp)
...
# a0 = sprite (com cabe�alho), a1 = x, a2 = y
draw_sprite8x8:
    la t0, base_frame_A
    lw s0, 0(t0)
    addi a0, a0, 12
...
# Fun��o da mana do player: desenha a barra de mana (preenche proporcional)
draw_mana_bar:
    addi sp, sp, -12
    sw s1, 8(sp)
    sw s2, 4(sp)
...
# Fun��o de dano do player: aplica hit, mata inimigos da tela e reinicia
mago_atingido:
    addi sp, sp, -4
    sw ra, 0(sp)
    la t0, escudo_timer
